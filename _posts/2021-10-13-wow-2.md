...
Our naive reimplementation happens to outperform the original in almost all the data splits
used in the original work. Very interestingly, in the case when the knowledge to use is predicted by the
network (Predicted Knowledge) and not provided by the task (Gold Knowledge), our implementation
closes the gap massive with respect to the original implementation. 
