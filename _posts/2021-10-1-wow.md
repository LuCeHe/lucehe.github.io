...
We only translated into tensorflow the End2End variant. The best performances we achieve are a validation perplexity 
of 11.3 (the lower the merrier) and a 
...
choice of initializers and the custom knowledge dropout that we implemented since we did not find it in the original 
